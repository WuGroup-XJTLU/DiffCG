...
def initialize_inter_radial_distribution_fun(box, displacement_fn, inter_rdf_params):
    """
    Initializes a function that computes the radial distribution function (RDF) for a single state.
    ...
    def rdf_compute_fun(state, **unused_kwargs):
        ...
        if isinstance(state,simulate.NPTNoseHooverState):
            box_ = simulate.npt_box(state)
            total_vol = box_volume(box_, spatial_dim)  # volume of partition
            mean_pair_corr = pair_corr_fun(R, box=box_)
        else:
            total_vol = box_volume(box, spatial_dim)  # volume of partition
            mean_pair_corr = pair_corr_fun(R)
...
def initialize_angle_distribution_fun(displacement_fn, adf_params):
    """
    ...
    distance_metric = custom_space.map_vector(displacement_fn)
    bin_size = jnp.diff(adf_bin_boundaries)

    def angle_corr_fun(R):
        """Compute adf contribution of each triplet."""

        R_kj=distance_metric(R[angle_top[:,2],:],R[angle_top[:,1],:])
        R_ij=distance_metric(R[angle_top[:,0],:],R[angle_top[:,1],:])
...
def initialize_dihedral_distribution_fun(displacement_fn, ddf_params):
    """
    Initializes a function that computes the angular distribution function (ADF) for a single state.
    ...
    def angle_corr_fun(R):
        ...
        R_ab=distance_metric(R[angle_top[:,0],:],R[angle_top[:,1],:])
        R_bc=distance_metric(R[angle_top[:,1],:],R[angle_top[:,2],:])
        R_cd=distance_metric(R[angle_top[:,2],:],R[angle_top[:,3],:])
